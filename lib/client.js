...
Client.prototype.printQLen = function() {
        //console.log('SQL Q len ' + (new Date().getTime() / 1000)+ ' ' + this._queue.length);
...
Client.prototype._enqueue = function(fn, delegate) {
  ...
  this._queue.push({fn: fn, delegate: delegate, startTS: new Date()});
...
Client.prototype._dequeue = function() {
  var thisItem = this._queue.shift();
  if (thisItem.startTS) {
  	var sqlstmt = '';
  	if (thisItem.delegate && (thisItem.delegate instanceof Query)) {
		sqlstmt = thisItem.delegate.sql;
	}
	var timeTaken = new Date().getTime() - thisItem.startTS.getTime();
	if (timeTaken > 3000) {
  		console.log('INFO: query -' + sqlstmt + '- took ' + timeTaken + ' ms');
	}
  } else {
  	console.log('ERROR: Mysql startTS not found');
  }
  ...
  //add execTS to startTime
  var nextItem = this._queue[0];
  if (nextItem.startTS) {
  	nextItem.execTS = new Date();
  }
  this._queue[0].fn();
};

Client.prototype._printExecTime = function() {
  var currItem = this._queue[0];
  if (currItem.startTS) {
  	var sqlstmt = '';
  	if (currItem.delegate && (currItem.delegate instanceof Query)) {
		sqlstmt = currItem.delegate.sql;
	}
	var timeTakenExec = 0;
	if (currItem.execTS) {
		timeTakenExec = new Date().getTime() - currItem.execTS.getTime();
	}
	if (timeTakenExec > 100) {
  		console.log('INFO: query -' + sqlstmt + '- took ' +
  					timeTakenExec + ' exec ms');
  	}
  } else {
  	console.log('ERROR: Mysql startTS exec not found');
  }
}
